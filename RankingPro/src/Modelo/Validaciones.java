...
public class Validaciones {
    ...
    public static String getValidar(String cuenta,
            ...
            String telefono,
            String previamente,
            String puntos, ObservableList<Ranking> listaRanking, int i) {
        String resultado = "";
        if(cuenta == null || cuenta.isEmpty()){
            resultado = resultado + "El campo CUENTA es obligatorio \n";
        }else{
        	if(i == 1){
        		for(int cuenta2 = 0; cuenta2 < listaRanking.size(); cuenta2++){
                    if(cuenta.equalsIgnoreCase(listaRanking.get(cuenta2).getCuenta())){
                        resultado = resultado + "El número de CUENTA ingresado ya existe \n";
                        System.out.println(listaRanking.get(cuenta2).getCuenta());
                    }
                }
        	}
        }
